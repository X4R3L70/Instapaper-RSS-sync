...
# --- CONFIGURATION (Keep your existing environment variables) ---
...
def get_oauth_token():
    """Obtains the OAuth token and secret."""
            ...
            if url not in tracked_data:
                try:
                    ...
                    if res.status_code == 200:
                        data = res.json()
                        # Instapaper returns a list; bookmark is usually index 0
...
def cleanup_old_articles(auth, tracked_data):
        ...
        if list_res.status_code == 200:
            for item in list_res.json():
                if isinstance(item, dict) and item.get('type') == 'bookmark':
                    # Store by ID as string for consistent lookup
    ...
    for url, info in tracked_data.items():
        # Keep items that are less than 24h old
        if current_time - info['added_at'] < seconds_in_24h:
            updated_tracked_data[url] = info
            continue
        
        # Process items older than 24h
        bookmark_id = str(info.get('id'))
        
        if bookmark_id in unread_bookmarks:
            bookmark = unread_bookmarks[bookmark_id]
            # If not starred, delete it
            if str(bookmark.get('starred')) == '0':
                print(f"Deleting unliked article: {url}")
                del_res = requests.post("https://www.instapaper.com/api/1.1/bookmarks/delete", 
                                        auth=auth, data={'bookmark_id': bookmark_id})
                if del_res.status_code != 200:
                    # If deletion fails, keep it in JSON to try again next hour
                    updated_tracked_data[url] = info
            else:
                print(f"Keeping liked article: {url}")
        else:
            print(f"Article {url} already archived/moved.")
